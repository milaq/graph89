...
TIEXPORT1 int TICALL ticables_library_init(void)
{
	...
	hDll = GetModuleHandle("libticables2-5.dll");
...
TIEXPORT1 CableHandle* TICALL ticables_handle_new(CableModel model, CablePort port)
{
	...
	for(i = 0; cables[i]; i++)
	{
		...
		{
			c = (CableFncts*)malloc(sizeof (CableFncts)); //release
			memcpy(c, cables[i], sizeof (CableFncts));
			handle->cable = c;

			//handle->cable = c;//(CableFncts *)cables[i];
			break;
		}
	}
